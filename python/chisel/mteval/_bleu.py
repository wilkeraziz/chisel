...
def dict2array(sparse_features, dimension):
    ...
    for k, v in sparse_features.iteritems():
        dok[k] = v
    return dok
...
class DecodingBLEU(object):
    ...
    >>> Hypothesis = namedtuple('Hypothesis', ['leaves'])
    >>> H = [Hypothesis('the black dog barked at the black cat'.split()), \
            Hypothesis('black dog barks at black cat'.split())]
    ...
    def __init__(self, hypotheses, posterior, max_order=4, smoothing='ibm'):
        ...
        self.sparse_features_ = [make_features(Dy.leaves, self.ngram_factory_) for Dy in hypotheses]

        self.doks_ = np.array([make_arrays(features, self.ngram_factory_) for features in self.sparse_features_])
        self.len_ = np.array([len(Dy.leaves) for Dy in hypotheses], float)
    ...
    def clipped_counts(self, c, r):
        if self.cc_ is None:
            ...
            n = self.max_order_
            cc = [[None] * m for _ in xrange(m)]  # clipped counts
            for i in range(m):
...
class TrainingBLEU(object):
    """
    >>> from collections import namedtuple
    >>> # mock reference and hypothesis classes
    >>> Reference = namedtuple('Reference', ['leaves'])
    >>> Hypothesis = namedtuple('Hypothesis', ['leaves'])
    >>> # data
    >>> R = [Reference('the black dog barked at the black cat'.split()), \
            Reference('the dog barked at the cat'.split())]
    >>> H = [Hypothesis('the black dog barked at a black cat'.split()), \
            Hypothesis('black dog barks at black cat'.split()), \
            Hypothesis('the black dog barked at the black cat'.split())]
    >>> # BLEU
    >>> tbleu = TrainingBLEU(R, H)
    >>> tbleu.clipped_counts(0)
    array([ 0.,  7.,  5.,  3.,  2.])
    >>> tbleu.clipped_counts(1)
    array([ 0.,  5.,  2.,  0.,  0.])
    >>> tbleu.clipped_counts(2)
    array([ 0.,  8.,  7.,  6.,  5.])
    >>> tbleu.totals(0)
    array([ 0.,  8.,  7.,  6.,  5.])
    >>> tbleu.totals(1)
    ...
    >>> tbleu.totals(2)
    array([ 0.,  8.,  7.,  6.,  5.])
    >>> tbleu.closest_length(0)
    8.0
    >>> tbleu.closest_length(1)
    6.0
    >>> tbleu.closest_length(2)
    8.0
    >>> tbleu.bleu(0)
    0.5946035575013605
    >>> tbleu.bleu(1)
    ...
    def __init__(self, references, hypotheses, max_order=4, smoothing='ibm'):
        self.max_order_ = max_order
        self.ngram_factory_ = NGramFactory(max_order)
        # feature extraction
        r_features = [make_features(ref.leaves, self.ngram_factory_) for ref in references]
        h_features = [make_features(hyp.leaves, self.ngram_factory_) for hyp in hypotheses]
        # features as matrices
        # 1) hypotheses
        self.h_counts_ = np.array([make_arrays(features, self.ngram_factory_) for features in h_features])
        # 2) references: compute the maximum ref counts for each ref ngram
        ref_counts = np.array([make_arrays(features, self.ngram_factory_) for features in r_features])

        # something like
        self.ref_counts = ref_counts
        self.max_ref_counts_ = np.array([reduce(np.maximum, (ri_counts[k] for ri_counts in ref_counts))
                                         for k in range(self.max_order_ + 1)])

        # 3) length
        self.r_len_ = np.array([len(ref.leaves) for ref in references], float)
        self.h_len_ = np.array([len(hyp.leaves) for hyp in hypotheses], float)

        # clipped counts (cc), total counts (tc) and closest length are computed lazily
        self.cc_ = None
        self.tc_ = None
        self.closest_len_ = None
    ...
    def clipped_counts(self, c):
        if self.cc_ is None:
            self.cc_ = np.array([[np.minimum(hi_counts[k], self.max_ref_counts_[k]).sum()
                                  for k in range(self.max_order_ + 1)]
                                 for hi_counts in self.h_counts_])
        return self.cc_[c]

    def totals(self, c):
        if self.tc_ is None:
            self.tc_ = np.array([[counts_k.sum() for counts_k in hi_counts] for hi_counts in self.h_counts_])
        return self.tc_[c]

    def closest_length(self, c):
        if self.closest_len_ is None:
            r_sorted = self.r_len_.copy()
            r_sorted.sort()
            self.closest_len_ = [closest(l, r_sorted) for l in self.h_len_]
        return self.closest_len_[c]

    def bleu(self, c):
        """Compare candidate c to the references in terms of BLEU"""
        return BLEU.bleu(r=self.closest_length(c),
                         c=self.h_len_[c],
                         cc=self.clipped_counts(c),
                         tc=self.totals(c),
                         n=self.max_order_,
                         smoothing=self.smoothing_)
